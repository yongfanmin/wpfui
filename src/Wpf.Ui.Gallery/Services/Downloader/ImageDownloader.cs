...
public class ImageDownloader : IImageDownloader
{
    ...
    public static Uri ParseImageUrl(string imageUrl, string defaultScheme = "http")
    {
    ...
    {
        return DownloadImageAsync(imageUrl, directoryPath, fileName, cancellationToken).Result;
    }
        ...
        try
            {
                ...
                await using (var networkStream = await _httpClient.GetStreamAsync(ParseImageUrl(imageUrl), cancellationToken))
                {
                    // 将网络流的所有内容异步复制到内存流
