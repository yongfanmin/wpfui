...
public class OrderPrintBatch
{
    ...
    public ProduceBatchStatus ProduceBatchStatus { get; set; } = ProduceBatchStatus.锁定生产中;
    
    public static OrderPrintBatch ConstructByJson(JsonNode jsonNode)
    {
        ...
        try
        {
            return JsonSerializer.Deserialize<OrderPrintBatch>(jsonNode.ToString());
        }
        catch (Exception e)
        {
            ...
            orderPrintBatch.OrderNo = (string?)jsonNode["order_no"] ?? string.Empty;
            ...
            if (batchNoNode != null)
            {
                ...
                {
                    orderPrintBatch.BatchNum = batchNum;
                }
                ...
                {
                    // 所有尝试都失败了，赋予一个安全的默认值
                    orderPrintBatch.BatchNum = 0L;
                }
            }
            return orderPrintBatch;
        }
        
    }

    public static List<OrderPrintBatch> ConstructByArrayJson(List<JsonNode?> jsonObjectListString)
    {
